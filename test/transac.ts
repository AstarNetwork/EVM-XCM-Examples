...
describe("Reserve Asset Transfer Contract", function () {
    ...
    before("Setup env", async function () {
        ...
        transferContract = await ethers.getContractFactory("RemoteTransac", {
            libraries: {
    ...
    it("Should perform a reserve asset transfer", async function () {
        ...
        // Balance Before of bob stash
        const { nonce, data: balance } = await shibuya_api.query.system.account("bgrbE6JxaHBk2tS7JH4rjcicyMki4t1E9M6YivFzCUq2YDg");

        await transferContract.connect(alith).remote_transact({
            gasLimit: 346804
        });

        await waitFor(60 * 1000);
        await expect((await await shibuya_api.query.system.account("bgrbE6JxaHBk2tS7JH4rjcicyMki4t1E9M6YivFzCUq2YDg")).balance.toString()).to.equal(balance.add(new BN('100000000000000000000')).toString())
    });
});